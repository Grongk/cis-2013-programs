...
{
    var stringPassword, arrayBadPasswords, stringMessage;
    ...
    if (stringPassword.length < 8) // checks to see if the password is greater than 8 characters
    {
        stringMessage = ("Must be at least 8 characters");
    }
    else
    for(var intCount = 0; intCount< arrayBadPasswords.length; intCount++) //starts the loop that goes through all of the bad passwords to check to see if the user's password is the same
    {
        if (stringPassword === arrayBadPasswords[intCount])
        {
            stringMessage = ("Bad Password :( ");
        break;} //breaks out of the loop once it goes through all of the passwords
        else
        {
            stringMessage = ("GOOD PASSWORD!!!!!");
        }
    }
    $("output").value=stringMessage;  //sends the message back to the html to show the user that their password is good!
}; 
